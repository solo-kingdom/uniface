...
```
pkg/storage/kv/              # KV 存储接口 (Redis, Aerospike, BoltDB)
...
详细结构参见 `docs/PROJECT_STRUCTURE.md`。
...
使用 OpenSpec spec-driven 工作流进行功能开发：
- `/opsx:propose <描述>` - 创建变更提案（自动生成 proposal、design、specs、tasks）
- `/opsx:apply` - 按任务实施变更
- `/opsx:archive` - 归档完成的变更
- `/opsx:explore` - 探索模式和思考伙伴

能力规格（`openspec/specs/`）定义各功能的接口契约和行为规格，是变更的基线。
...
### 文档
- 使用中文
- 三层结构: `openspec/specs/`(能力规格) → `docs/`(设计) → `pkg/`(实现)，路径保持一致
- 文档索引参见 `docs/README.md`
...
注意: `pkg/storage/kv/redis`、`pkg/storage/kv/aerospike`、`pkg/storage/kv/boltdb`、`pkg/rpc/governance/config/consul` 是独立 Go 子模块。
