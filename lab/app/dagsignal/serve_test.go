...
// TestServe_RegisterRoutes 验证装配流程：
// buildRuntime + NewService + Register 不报错。
// 端到端 HTTP 行为见 handler_test.go（沙箱不允许绑定端口）。
...
func TestServe_RejectsUnsupportedStore(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "unsupported dagsignal store") {
		t.Fatalf("error = %q, want unsupported dagsignal store", err.Error())
	}
}
...
// TestLoadConfig_AppliesEnvOverrides 验证 LAB_DAGSIGNAL_STORE / LAB_DAGSIGNAL_FIXTURES_DIR 覆写。
func TestLoadConfig_AppliesEnvOverrides(t *testing.T) {
	...
	if err := os.WriteFile(cfgPath, []byte("dagsignal:\n  store: memory\n  fixtures_dir: ignored-by-env\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	t.Setenv("LAB_CONFIG", cfgPath)
	t.Setenv("LAB_DAGSIGNAL_STORE", "memory")
	t.Setenv("LAB_DAGSIGNAL_FIXTURES_DIR", "app/dagsignal/fixtures/graphs")
	...
	if cfg.FixturesDir != "app/dagsignal/fixtures/graphs" {
		t.Fatalf("FixturesDir = %q, want app/dagsignal/fixtures/graphs", cfg.FixturesDir)
	}
}
...
func TestLoadConfig_AppliesDefaults(t *testing.T) {
	...
	t.Setenv("LAB_CONFIG", cfgPath)
	t.Setenv("LAB_DAGSIGNAL_STORE", "")
	t.Setenv("LAB_DAGSIGNAL_FIXTURES_DIR", "")
